  ...
  class ContentComponent extends HTMLElement{
    ...
    connectedCallback() {
        ...
        this._shadowRoot.querySelector('#postbtn').addEventListener('click',function(){
            ...
            if(title == "" || title <3 || title >20){errormsgsection.innerText = "Enter Valid Title"}
            else if(description == ""){errormsgsection.innerText = "Enter Valid Description"}
            else if(category == ""){errormsgsection.innerText = "choose Valid category"}
            else{
    ...
    //close post modal
        ...
        Array.from(closeElements).forEach(function(element) {
            element.addEventListener('click',() => {postmodal.style.display = "none";});
        });
    }
    getCurrentDate() {
        var today = new Date();
